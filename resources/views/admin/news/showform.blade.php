...
@section('header', 'Create new News')

@section('main-content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Create <i>News</i></h3>
        </div>
        <form action="{{ route('admin.news.store') }}" enctype="multipart/form-data" method="post">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="text" type="text" class="form-control" placeholder="TEXT" rows="5" required>{{ old('text') }}</textarea>
                    @error('text')
                    <span class="text-danger mt-2">{{ $message }}</span>
                    @enderror
                </div>
